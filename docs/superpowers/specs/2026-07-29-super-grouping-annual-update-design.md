...
**Regulatorische Grundlage (2026-07-30, Domain-Review):** Die Blockade ist keine Vorsichtsmaßnahme, sondern bildet eine echte prozedurale Unmöglichkeit ab. Die Rechtsgrundlage für ein Type-IA-only-Bündel über mehrere Zulassungen ist **Art. 7(2)(b) VO (EG) 1234/2008** (eine einzige Notifikation für dieselbe Type-IA-Änderung über mehrere Zulassungen desselben Inhabers) — **nicht** Art. 20 (Worksharing, das ausdrücklich nur Type IB/II abdeckt). Art. 20(3) enthält eine explizite CAP/NAP-Brückenregel (EMA/CHMP als Referenzbehörde, sobald eine zentral zugelassene Zulassung beteiligt ist; sonst CMDh-benannte RMS) — Art. 7(2)(b) kennt keine solche Brücke und keinen Mechanismus zur Benennung einer Referenzbehörde über CP und national/MRP-DCP hinweg. Eine einzelne Notifikation kann nicht gleichzeitig bei der EMA und bei nationalen Behörden/RMS eingehen. Das begründet auch die Asymmetrie zur Kapitel-C-Regel: Kapitel C bei ≥2 RMS bleibt einreichbar (nur pro RMS aufgeteilt) — daher Warnung; CP+national/MRP-DCP hat dagegen gar kein Verfahren, über das die Notifikation gemeinsam eingereicht werden könnte — daher Blockade statt Warnung. *(Hinweis: Artikelnummern vor Verwendung in nutzersichtbaren Texten gegen die aktuelle CMDh-Q&A zu Variations verifizieren.)*

**Nachtrag (2026-07-30, Modus-Wechsel-Lücke geschlossen):** Ein Domain-Review deckte auf, dass eine unter Worksharing gebaute gemischte Verfahrensliste (CP + national/MRP-DCP, dort legal) einen zwischenzeitlichen `mode = null` überleben und beim Wechsel in Super-Grouping unvalidiert übernommen werden konnte — die Chip-Sperre in `procEditor()` verhindert nur *neue* Verstöße am Picker, nicht bereits bestehende. Fix: `rerender()` löst bei aktivem Super-Grouping die Verfahrensliste bei jedem Render auf die **Familie des Basisverfahrens** auf (`state.procedure.kind`) — inkompatible Zusatzverfahren werden automatisch entfernt, da das Basisverfahren zuerst konfiguriert wurde und den Ausschlag gibt. Idempotent, sobald die Liste bereits konsistent ist.
